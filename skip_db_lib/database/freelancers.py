...
class FreelancerDatabase:
    ...
    @staticmethod
    def _adapt_for_bulkwrite(fields_to_update: Dict[str, Any], _filter: Dict[str, Any]) -> Optional[List[Dict[str, Any]]]:
        """
        ...
        for field, val in fields_to_update.items():
            if isinstance(val, dict):
                if len(val) == 0 or len(val) > 1:
                    raise ValueError(f"'{field}' can't be empty or contain more then 1 key/operation ['add', 'rem']")

                op, data = val.popitem()
                writes.append(
                    UpdateOne(_filter, FreelancerDatabase._build_array_update_write(field, op, data))
                )
            else:
        ...
        if len(basic_update_write.get("$set")) > 0:
            writes.append(
                UpdateOne(_filter, basic_update_write)
            )
